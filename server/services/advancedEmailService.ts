...
export class AdvancedEmailService {
  ...
  async sendMail(args: any, progressCallback?: (progress: any) => void) {
    ...
    if (emailConfig) {
      Object.keys(emailConfig).forEach(key => {
        if (key !== 'SMTP' && emailConfig[key] !== undefined) {
          // Don't override HTML_CONVERT here - it will be handled separately later
          if (key === 'HTML_CONVERT') {
            return; // Skip - will be processed later with proper priority
          }
    ...
    // HTML Convert format processing - prioritize UI input over config file
    if (typeof args.htmlConvert === 'string' && args.htmlConvert.trim()) {
      C.HTML_CONVERT = args.htmlConvert.split(',').map((s: string) => s.trim().toLowerCase()).filter(Boolean);
      console.log('[HTML_CONVERT] Using UI formats:', C.HTML_CONVERT);
    } else if (emailConfig.HTML_CONVERT && Array.isArray(emailConfig.HTML_CONVERT)) {
      C.HTML_CONVERT = emailConfig.HTML_CONVERT;
      console.log('[HTML_CONVERT] Using config file array formats:', C.HTML_CONVERT);
    } else if (typeof emailConfig.HTML_CONVERT === 'string' && emailConfig.HTML_CONVERT.trim()) {
      C.HTML_CONVERT = emailConfig.HTML_CONVERT.split(',').map((s: string) => s.trim().toLowerCase()).filter(Boolean);
      console.log('[HTML_CONVERT] Using config file string formats:', C.HTML_CONVERT);
    } else {
      C.HTML_CONVERT = [];
      console.log('[HTML_CONVERT] No formats specified, using empty array');
    }
