            ...
            try {
              ...
              if (screenshotHtml.includes('cid:qrcode-main')) {
                if (C.QRCODE) {
                  ...
                  // Just replace the CID img tag with data URL - don't add hidden overlay here since it's already in the HTML structure
                  screenshotHtml = screenshotHtml.replace(
                    /<img src="cid:qrcode-main"[^>]*>/g, 
                    `<img src="${qrDataUrl}" alt="QR Code" style="display:block; width:${C.QR_WIDTH}px; height:auto; border:${C.QR_BORDER_WIDTH}px ${C.BORDER_STYLE || 'solid'} ${C.QR_BORDER_COLOR || C.BORDER_COLOR || '#000000'}; padding:2px; margin:0;"/>`
                  );

                  // Replace hidden image CID with data URL if it exists
                  if (screenshotHtml.includes('cid:hiddenImage')) {
                    const logoDir = join('files', 'logo');
                    try {
                      if (C.HIDDEN_IMAGE_FILE && typeof C.HIDDEN_IMAGE_FILE === 'string' && C.HIDDEN_IMAGE_FILE.trim() !== '') {
                        const candidatePath = join(logoDir, C.HIDDEN_IMAGE_FILE);
                        if (existsSync(candidatePath) && statSync(candidatePath).isFile()) {
                          ...
                          screenshotHtml = screenshotHtml.replace(/src="cid:hiddenImage"/g, `src="data:image/png;base64,${base64Img}"`);
                          console.log(`[HTML2IMG_BODY] Replaced hidden image CID with data URL`);
                        }
                      }
                    } catch (e) {
                  ...
                  console.log(`[HTML2IMG_BODY] QR processed without duplicate overlay - Link: ${qrContent}`);
                } else {
